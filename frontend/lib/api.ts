...
export async function searchPerformers(payload: { from_city: number; to_city: number; mode: "exact" | "partial" }): Promise<any[]> {
  return j<any[]>(
...
export async function getDeals(params: { limit?: number; offset?: number; performer_id?: number } = {}): Promise<any[]> {
  const usp = new URLSearchParams();
  if (params.limit) usp.set("limit", String(params.limit));
  if (params.offset) usp.set("offset", String(params.offset));
  if (params.performer_id) usp.set("performer_id", String(params.performer_id));
  return j<any[]>(await fetch(`${API_BASE}/deals?${usp.toString()}`, { cache: "no-store" }));
}
